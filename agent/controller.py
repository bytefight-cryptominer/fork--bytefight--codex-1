...
class PlayerController:
    ...
    def play(
        ...
        def count_same_hill_neutrals(r, c, hill_id, exclude=None):
            count = 0
            for dr, dc in DR:
                nr, nc = r + dr, c + dc
                if exclude is not None and (nr, nc) == exclude:
                    continue
                if not valid(nr, nc):
                    continue
                pcell = board.cells[nr][nc]
                if pcell.hill_id == hill_id and pcell.owner_parity == 0:
                    count += 1
            return count

        # --- Erase step for hill cells with opponent paint ---
        # Score all legal erase targets, then take the best combo instead of the first hit.
        if stamina >= 50:  # 40 erase + 10 buffer
            best_erase_only = None
            best_erase_score = -1
            best_combo = None
            best_combo_score = -1
            ...
            for dr, dc in DR:
                ...
                if (ecell.hill_id and ecell.hill_id != 0 and
                    ...
                    erase_only_score = (
                        count_same_hill_neutrals(nr, nc, hill_id) * 10
                        + count_paintable(nr, nc)
                    )
                    if erase_only_score > best_erase_score:
                        best_erase_score = erase_only_score
                        best_erase_only = Action.Move(DIR_MAP[(dr, dc)], move_type=MoveType.ERASE)
                    ...
                    if stamina >= 65:
                        ...
                        if best_exit:
                            ...
                            same_hill_neutrals = count_same_hill_neutrals(
                                nr,
                                nc,
                                hill_id,
                                exclude=(off_r, off_c),
                            )
                            combo_score = same_hill_neutrals * 10 + best_exit_score
            ...
            if best_combo is not None:
                return best_combo
            if best_erase_only is not None:
                return [best_erase_only]
