...
class PlayerController:
    ...
    def _find_move_candidates(self, board, me, parity, rows, cols,
                              ...
                              goal_cells=None, goal_dist=None, unsafe_cells=None):
        """
        ...
        candidates = []
        start_goal_dist = goal_dist.get((start.r, start.c), 99) if goal_dist else 99
        ...
        for d in dirs:
            ...
            if preferred_dir and d == preferred_dir:
                score += 8.0
            if goal_dist:
                step_goal_dist = goal_dist.get((nl.r, nl.c), 99)
                if step_goal_dist < 99:
                    score += max(0, 8 - step_goal_dist) * 2.2
                    score += max(0, start_goal_dist - step_goal_dist) * 4.0
            if goal_cells and (nl.r, nl.c) in goal_cells:
                score += 18.0

            post_stamina = me.stamina - GameConstants.EXTRA_MOVE_COST
            if cell.powerup:
                post_stamina = min(
                    me.max_stamina,
                    post_stamina + GameConstants.STAMINA_POWERUP_AMOUNT,
                )
            immediate_paints = 0
            goal_paints = 0
            for d3 in Direction.cardinals():
                ...
                if board.oob(pt):
                    continue
                if not self._can_paint(board.cells[pt.r][pt.c], parity):
                    continue
                immediate_paints += 1
                if goal_cells and (pt.r, pt.c) in goal_cells:
                    goal_paints += 1
                elif (pt.r, pt.c) in self.hill_set and board.cells[pt.r][pt.c].owner_parity != parity:
                    goal_paints += 1
            possible_paints = min(
                immediate_paints,
                max(0, post_stamina) // GameConstants.PAINT_STAMINA_COST,
            )
            post_stamina -= possible_paints * GameConstants.PAINT_STAMINA_COST
            score += possible_paints * 1.2
            score += min(goal_paints, possible_paints) * 2.5
            score += post_stamina * (0.08 if near_opp else 0.03)
            if unsafe_cells and (nl.r, nl.c) in unsafe_cells:
                if self._tile_secure(board, nl, parity, rows, cols):
                    score -= 4.0
                else:
                    score -= 18.0 if near_opp else 10.0
    ...
    def _simulate_candidates(self, board, me, parity, candidates, opp,
                             sim_turns, max_candidates, goal_cells=None,
                             goal_dist=None, unsafe_cells=None, near_opp=False):
        """
        ...
        for _, cand_dir in to_eval:
            ...
            ev = lb.evaluate()
            if goal_dist:
                end_key = (lb.my_r, lb.my_c)
                ev += max(0, 8 - goal_dist.get(end_key, 99)) * 8.0
                if goal_cells and end_key in goal_cells:
                    ev += 18.0
            if unsafe_cells and (lb.my_r, lb.my_c) in unsafe_cells:
                ev -= 20.0
            end_dist = abs(lb.my_r - lb.opp_r) + abs(lb.my_c - lb.opp_c) if lb.opp_r >= 0 else 99
            ev += lb.my_stamina * (0.7 if near_opp or end_dist <= 6 else 0.2)
            if ev > best_eval:
                best_eval = ev
                best_dir = cand_dir
    ...
    def play(
        ...
        if target:
            ...
            goal_cells, goal_dist = None, None
        unsafe_cells = self._opponent_unsafe_cells(board, player_parity, opp) if opp else None
