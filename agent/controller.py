...
class PlayerController:
    ...
    def play(
        ...
        def score_forecast(sim_board):
            ...
            for r in range(rows):
                for c in range(cols):
                    owner = sim_board.cells[r][c].owner_parity
                    if owner == player_parity:
                        owned_cells += 1
                        if sim_board.cells[r][c].hill_id:
                            hill_tiles += 1
                    elif owner == self.opp and sim_board.cells[r][c].hill_id:
                        hill_tiles -= 1

            for loc in sim_me.loc.square_region(2):
                if sim_board.oob(loc):
                    continue
                if sim_board.cells[loc.r][loc.c].owner_parity == player_parity:
                    local_control += 1
        ...
        # Use forecast_turn only for erase micro: same trigger, better local plan selection.
        if stamina >= 50:  # 40 erase + 10 buffer
            best_plan = None
            best_plan_score = None

            def consider_plan(actions):
                nonlocal best_plan, best_plan_score
                sim_board, ok = board.forecast_turn(player_parity, actions)
                if not ok:
                    return
                score = score_forecast(sim_board)
                if best_plan_score is None or score > best_plan_score:
            ...
            for dr, dc in DR:
                ...
                if (ecell.hill_id and ecell.hill_id != 0 and
                    ...
                    if nr == opp_r and nc == opp_c:
                        continue
                    consider_plan([Action.Move(DIR_MAP[(dr, dc)], move_type=MoveType.ERASE)])
